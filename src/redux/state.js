...
export let postChangeData = (postMessage) => {
  state.profileData.newPost = postMessage;
  callSubscriber();
};

export let addPostData = () => {
  let newId = state.profileData.wallData.length + 1;
  ...
  let newPost = {
    id: newId,
    text: state.profileData.newPost,
    likes: 0,
    usId: randomUsId(),
  };
  state.profileData.wallData.push(newPost);
